...
const microlamports = parseFloat(process.env.MICROLAMPORTS);
...
async function buyTokens(wallet) {
    ...
    try {
        // Removed the log for buying parameters
        ...
        if (data.status === 'success') {
            wallet.tokens = data.tokens; // Update wallet.tokens directly
            ...
            return wallet.tokens; // Return wallet.tokens if needed elsewhere
        } else {
...
async function sellTokens(wallet, retries = 0) {
    try {
        if (!wallet.tokens || wallet.tokens <= 0) {
            console.log(`Fetching token balance for wallet ${wallet.publickey} before selling...`);
            wallet.tokens = await checkTokenBalance(wallet);
            if (wallet.tokens <= 0) {
